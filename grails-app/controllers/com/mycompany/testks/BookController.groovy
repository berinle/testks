...
class BookController {
	...
	def save_m = {
		...
		if (bookInstance.save(flush: true)) {
			flash.message = "${message(code: 'default.created.message', args: [message(code: 'book.label', default: 'Book'), bookInstance.id])}"
			redirect(action: "show_m", id: bookInstance.id)
		}
		else {
			render(view: "create_m", model: [bookInstance: bookInstance])
		}
	}
		...
		if (bookInstance) {
			if (params.version) {
				...
				if (bookInstance.version > version) {
					...
					bookInstance.errors.rejectValue("version", "default.optimistic.locking.failure", [message(code: 'book.label', default: 'Book')] as Object[], "Another user has updated this Book while you were editing")
					render(view: "edit_m", model: [bookInstance: bookInstance])
					return
				}
			}
			bookInstance.properties = params
